...
function get_field_label($field_name) {
    ...
    $query = 'SELECT meta_value FROM wp_postmeta where meta_key = "_'.$field_name.'"';
    $field = $wpdb->get_var($query);

    $query = 'SELECT meta_value FROM wp_postmeta where meta_key = "'.$field.'"';
    $field = unserialize($wpdb->get_var($query));

    return $field["choices"];
}

function get_field_label_short($field_name) {

    global $post, $wpdb;

    $query = 'SELECT meta_value FROM wp_postmeta where meta_key = "'.$field_name.'"';
    $field = unserialize($wpdb->get_var($query));
